...
pi_result calculateKernelWorkDimensions(
    pi_kernel Kernel, ze_group_count_t &ZeThreadGroupDimensions,
    uint32_t (&WG)[3], pi_uint32 WorkDim, const size_t *GlobalWorkSize,
...
piEnqueueKernelLaunch(pi_queue Queue, pi_kernel Kernel, pi_uint32 WorkDim,
  ...
  auto result =
      calculateKernelWorkDimensions(Kernel, ZeThreadGroupDimensions, WG,
                                    WorkDim, GlobalWorkSize, LocalWorkSize);
  if (result != PI_SUCCESS) {
    return result;
  }
...
pi_result piextCommandBufferCreate(pi_context Context, pi_device Device,
  ...
  ZE_CALL(zeCommandListCreate, (Context->ZeContext, Device->ZeDevice,
                                &ZeCommandListDesc, &ZeCommandList));
  try {
    *RetCommandBuffer = new _pi_ext_command_buffer(Context, ZeCommandList,
                                                   ZeCommandListDesc, Desc);
  } catch (const std::bad_alloc &) {
    return PI_ERROR_OUT_OF_HOST_MEMORY;
  } catch (...) {
    return PI_ERROR_UNKNOWN;
...
pi_result piextCommandBufferNDRangeKernel(
  ...
  // Lock automatically releases when this goes out of scope.
  std::scoped_lock<pi_shared_mutex, pi_shared_mutex> Lock(
  ...
  zePrint("calling zeCommandListAppendLaunchKernel() with"
          "  ZeEvent %#lx\n",
          pi_cast<std::uintptr_t>(LaunchEvent->ZeEvent));
...
_pi_ext_command_buffer::_pi_ext_command_buffer(
    pi_context Context, ze_command_list_handle_t CommandList,
