...
, and is done when dynamic parameters are set as parameters to the kernel using
`handler::set_arg()`/`handler::set_args()`). It is valid for a node argument to
be registered with more than one dynamic parameter instance.
...
Parameters to individual nodes in a graph in the `executable` state
...
node HostTaskNode = myGraph.add([&](handler& cgh) {
    cgh.host_task([=]() {
            Ptr[i] = N;
        }
    });
...
----
  ...
  event Event = Queue.submit(
        [&](handler &CGH) { CGH.ext_oneapi_graph(ExecGraph); });
  Event.wait;
...
// First node uses ptrX as an input & output parameter,
// with operand myScalar as another argument.
...
auto execGraph = myGraph.finalize(
                  {sycl_ext::property::graph::updatable});

// Execute graph, then update without needing to wait
