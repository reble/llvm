...
public:
  ...
  void update_dynamic_parameter(const std::vector<node>& nodes,
...
----
|Associate a dynamic parameter with this node, so that an updated to the
 dynamic parameter
...
with this list of nodes will maintain the graphs data dependencies.
...
| Updates a parameter in target nodes with a new value. This new value will
...
* `nodes` - The node in this graph which will have their parameter value updated.
...
* Throws with error code `invalid` if any node in `nodes` is not part of the
  graph.
* Throws with error code `invalid` if `dynamicParam` is not registered with any
  node in `nodes`.
