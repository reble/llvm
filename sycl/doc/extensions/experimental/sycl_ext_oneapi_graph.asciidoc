...
because SYCL kernel arguments don't have indices and ordering of lambda captures
isn't defined, so to we need a way to tie the underlying node argument to
a `dynamic_parameter` object.
...
data dependency, and prevents unexpected behaviour.
