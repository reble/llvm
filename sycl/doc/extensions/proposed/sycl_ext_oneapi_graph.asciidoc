...
Host code within a command group function object is evaluated once either when
submitting a command to a queue in recording mode or when adding a node to a
graph explicitly. This behaviour is in keeping with the SYCL 2020 specification
but may have implications for command group functions containing arbitrary host
code. This could affect the behaviour of captured code due to the delayed
