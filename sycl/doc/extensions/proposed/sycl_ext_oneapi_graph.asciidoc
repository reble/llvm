...
* The code that is executed in host-task or device kernel nodes may be
  different between graphs. However, the update will only reflect the new code
  in `graph` host-tasks nodes, as it doesn't need device-side interaction. Any
  changes to executable code in `graph` kernel nodes will be ignored by the
  update.
