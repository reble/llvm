...
* `syclDevice` - Device this command will run on, if not a host task.
...
* Throws asynchronously with error code `invalid` if the handler is submitted
...
result in an asynchronous exception.
...
the benefits of defining a execution graph ahead of submission to be extended
...
it is the responsibility of the user to decide the device each command will run
on, not the SYCL runtime.
...
SYCL reductions require a work-group/sub-group size to be known, which is
normally gathered by the runtime from the device associated with the queue.
