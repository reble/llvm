...
copy of that data inside the buffer. To illustrate, consider an example like this:
...
case, he or she may use the graph::no_host_copy property to avoid the internal
copy. Users can opt-out of this behavior by passing the property `graph::no_host_copy`
to `command_graph::begin_recording()` or the constructor of the `command_graph`
the commands will be recorded to. Passing the property to `begin_recording()`
will prevent host copies only for commands recorded before `end_recording()` is
called for a given queue. Passing the property to the `command_graph` constructor
will prevent host copies for all commands recorded to the graph.
