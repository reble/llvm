...
`free` node is executed and after the `malloc` node is executed. When a `command_graph` contains
any nodes that allocate memory, only one executable graph object can be alive at the same time.
...
* `kind` - Specifies the type of USM to allocate
...
* Throws synchronously with error code `invalid` if the pointer has already freed
  or is an invalid address.
...
=== Storage Lifetimes and Accessibility
...
USM allocations are accessible from all nodes that are located after the malloc
and before an (optional) free node. When the execution is in process accessing 
the memory from anywhere else inside and outside the graph is undefined behavior.
If the memory is not freed inside a graph, it's the users resposiblity to release 
...
When the executable graph is executed again the underlying memory might be reused.
...
int main() {
  ...
  auto node_x = g.add_malloc<float>(x, sycl::usm::alloc::device, n);
  auto node_y = g.add_malloc<float>(y, sycl::usm::alloc::device, n);
  auto node_z = g.add_malloc<float>(z, sycl::usm::alloc::device, n);
