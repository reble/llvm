...
accessors. There is also the ability for the SYCL runtime to track USM
allocation dependencies based on the pointers used in recorded command groups,
and construct an edge between nodes where the same USM allocation is used.
|===
