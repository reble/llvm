...
----
|Adding a node that encapsulates a USM allocation operation.
...
* Throws synchronously with error code `invalid` if the pointer has already been freed
...
and before an (optional) free node. While the graph is being executed accessing 
the memory from anywhere else inside or outside the graph is undefined behavior.
...
When the executable graph is executed again the previously allocated memory may be reused.
