...
code which relies on the copy-back on destruction behaviour of buffers.
...
When using the explicit graph API this restriction on buffers does not apply and
the above error will not be thrown. However, because of the extension of storage
lifetimes users should avoid relying on the copy-back mechanism in code using the
explicit API.
